...
def save_model(args, model, accuracy):
    ...
    logger.info("Saved the best model with {:.4f} to [DIR: %s]".format(accuracy), args.output_dir)
...
def setup(args):
    ...
    if args.use_context:
        ...
        assert args.train_batch_size % args.context_tokens == 0
        args.train_batch_size = args.train_batch_size//args.context_tokens
        assert args.num_epochs % args.context_tokens == 0
        args.num_epochs = args.num_epochs // args.context_tokens
...
def valid(args, model, writer, test_loader, global_step, labels=None):
    # Validation!
    ...
    if args.use_context:
        return 0.2*correct_meter_subset['jpg'] + 0.8*accuracy
    else:
        return accuracy
...
def train(args, model):
    ...
    # Prepare dataset
    train_loader, test_loader, labels = get_loader(args)
    ...
    t_total = args.num_epochs * len(train_loader)
    if args.use_context:
        sample_per_epoch = t_total / (args.context_tokens*args.num_epochs)
        val_point_list = list(range(args.num_epochs*args.context_tokens))
        val_point_list = [int(val_i*sample_per_epoch) for val_i in val_point_list]
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            x, y = batch
            if args.use_context:
                x, y = x.view(-1, args.emb_num), y.view(-1)
            loss = model(x, y)
            if len(args.gpus)>1:
                loss = loss.mean()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.use_context:
                    if global_step in val_point_list and args.local_rank in [-1, 0]:
                        accuracy = valid(args, model, writer, test_loader, global_step, labels=labels)
                        save_model_last(args, model, accuracy)
                        if best_acc < accuracy:
                            save_model(args, model, accuracy)
                            best_acc = accuracy
                        model.train()
                else:
                    if global_step % (args.eval_every* len(train_loader)) == 0 and args.local_rank in [-1, 0]:
                        accuracy = valid(args, model, writer, test_loader, global_step, labels=labels)
                        save_model_last(args, model, accuracy)
                        if best_acc < accuracy:
                            save_model(args, model, accuracy)
...
def main():
    ...
    parser.add_argument("--dataset", choices=["cifar10", "cifar100","fft1", "fft2", "fft3", "fft4"
                        , "fft5", "fft6", "rff"], default="fft1",
    ...
    parser.add_argument("--model_type", choices=["ViT-S_8","ViT-S_4","ViT-B_16", "ViT-B_32", "ViT-L_16",
                                                 "ViT-L_32", "ViT-H_14", "R50-ViT-B_16", "CNN", "ViT-S_1", "ViT-S_2",
